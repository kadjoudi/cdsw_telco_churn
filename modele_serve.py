...
def explain(args1):
    rows = args1.get("data").get("rows")
    colnames = args1.get("data").get("colnames")
    zipbObj = zip(colnames,rows)
    args = dict(zipbObj)
...
y={"data":{"colnames":["gender","tenure","MultipleLines","TotalCharges"], "coltypes":["STRING","DOUBLE","STRING","DOUBLE"], "rows":["Female",2,"No",151.65]}}
explain(y)
